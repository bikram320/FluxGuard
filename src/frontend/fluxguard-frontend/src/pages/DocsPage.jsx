        ...
        <div className="bg-surface border border-border rounded-lg overflow-hidden">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-display font-600 text-sm text-text">{title}</span>
                </div>
                {open ? <ChevronDown size={14} className="text-subtle" /> : <ChevronRight size={14} className="text-subtle" />}
            </button>
...
export default function DocsPage() {
        ...
        <div>
            <div className="mb-8">
                ...
                <p className="text-subtle text-xs mt-1">
                    Everything you need to integrate FluxGuard into your application.
                </p>
            </div>

            {/* Base URL banner */}
            <div className="bg-accent/5 border border-accent/20 rounded-lg px-4 py-3 mb-6 flex items-center justify-between">
                <div>
                    <p className="text-[10px] text-subtle uppercase tracking-wider mb-0.5">Base URL</p>
                    <p className="text-xs font-mono text-accent">{BASE_URL}</p>
                </div>
                            ...
                            <div className="space-y-3">
                                <div className="border border-border rounded-lg p-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-xs font-mono text-text">/auth/users/register</span>
                                    </div>
  ...
  "confirmPassword": "yourpassword"
                                ...
                                <div className="border border-border rounded-lg p-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="text-[10px] font-700 text-warning bg-warning/10 border border-warning/20 rounded px-1.5 py-0.5">POST</span>
                                        <span className="text-xs font-mono text-text">/auth/users/login</span>
                                    </div>
  ...
  "password": "yourpassword"
                        ...
                        <div>
                            ...
                            <div className="border border-accent/20 rounded-lg p-4 bg-accent/5">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="text-[10px] font-700 text-warning bg-warning/10 border border-warning/20 rounded px-1.5 py-0.5">POST</span>
                                    <span className="text-xs font-mono text-text">/api/fluxguard/security/check</span>
  ...
  "ipAddress":      "203.0.113.42",       // required — caller's IP
  "endpoint":       "/api/products",      // required
  "method":         "GET",                // required
  "createdAt":      "2024-01-15T10:30:00",// required
  "responseStatus": 200,                  // optional — pass 4xx for error-rate tracking
  "userAgent":      "Mozilla/5.0 ...",    // recommended — enables UA inspection
  "queryString":    "id=1 OR 1=1"         // recommended — enables injection detection
}`} />

                                <p className="text-[10px] text-subtle uppercase tracking-wider mb-1 mt-4">Response</p>
                                <CodeBlock lang="json" code={`// Allowed
{ "status": true,  "message": "Request allowed" }
...
async function fluxGuardCheck(req, res, next) {
  ...
  try {
    ...
    next(); // fail open — don't block if FluxGuard is unreachable
  }
}
...
public class FluxGuardFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        body.put("queryString",    req.getQueryString()); // SQLi/XSS check
